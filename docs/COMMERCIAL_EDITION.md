...
![超级截图 Windows 完整版真实主界面](images/paid-edition-main-zh.png)
...
**如果您移动或重命名了 EXE，请从新位置运行软件，然后点击紫色的“更新启动位置”按钮。** 上方的“需要更新”表示开机启动仍然开启，但 Windows 记录的还是原来的 EXE 路径。修复后，下次开机就会从新位置自动启动。橙色的“取消开机启动”按钮才是关闭自动启动。

After startup has been enabled, moving or renaming the EXE makes the top status show that an update is required. Startup is still enabled, but Windows is holding the old EXE path. Run the EXE from its new location and click **Update startup location**. The repaired path will be used automatically at the next Windows startup. **Disable startup** is the separate action that turns automatic startup off.
